...
command = argparse.ArgumentParser(prog='fasta_filter_to_description', \
    description=desc, usage='%(prog)s [options] key')
command.add_argument('-i','--input', default=sys.stdin, \
...
command.add_argument('-V','--reverse', action='store_true', \
    help='Return sequence without key in description')
command.add_argument('key', type=str, \
    help='Key words that must be present in description')
