...
command = argparse.ArgumentParser(prog='fasta_filter_to_region', \
    description=desc, usage='%(prog)s [options] fasta accessions:start-stop')
command.add_argument('-o','--output', default=sys.stdout, \
    type=argparse.FileType("w"), nargs='?', \
    help='Output result to, default:stdout')
command.add_argument('fasta', type=argparse.FileType("r"), \
    help='Fasta file to filter')
command.add_argument('access', metavar='accessions', \
    type=str, nargs='+', \
