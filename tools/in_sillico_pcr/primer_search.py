...
command = argparse.ArgumentParser(prog='primer_search.py', \
    description=desc, usage='%(prog)s [options] forward reverse database')
...
command.add_argument('-r', '--remove', action='store_true', \
    help='Remove primer from reported amplicons')
...
class  GassstResult():
    ...
    def __eq__(self, other):
        return isinstance(other, GassstResult) and self.primer==other.primer \
            and self.rev == other.rev and self.match==other.match \
...
class  Amplicon():
    ...
    def __init__(self, forw, reve):
        if forw.strand == reve.strand:
            raise Exception("Amplicon could be in inverse strand for reverse/forwar primer\n" + forw + reve )
        self.forw = forw
        self.reve = reve
