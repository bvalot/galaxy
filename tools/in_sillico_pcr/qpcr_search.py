...
gasst_exe = "Gassst"
nucleo = {'R':('A','G'), 'Y':('C','T'), 'S':('G','C'), 'W':('A','T'), 'K':('G','T'),\
              'M':('A','C'), 'B':('C','G','T'), 'D':('A','G','T'), 'H':('A','C','T'),\
              'V':('A','C','G'), 'N':('A','T','C','G')}

desc = "Search primer/probe on database and return position and errors."
command = argparse.ArgumentParser(prog='primer_search.py', \
    description=desc, usage='%(prog)s [options] forward probe reverse database')
command.add_argument('-o', '--out', nargs="?", \
    type=argparse.FileType("w"), default=sys.stdout, \
    help='Return amplicon position on file, default=stdout')
command.add_argument('-d', '--debug', nargs="?", \
    type=argparse.FileType("w"), default=None, \
    help='Conserve a copy of primer search, default No')
command.add_argument('-e', '--error', nargs="?", \
    type=int, default=1, \
    help='Maximun error allowed on match, default=1')
command.add_argument('-m', '--min', nargs="?", \
    type=int, default=50, \
    help='Min len amplicon size, default=50')
command.add_argument('-M', '--max', nargs="?", \
    type=int, default=200, \
    help='Max len amplicon size, default=200')
command.add_argument('-t', '--taxo', action='store_true', \
     help='Parse description to report species only')
command.add_argument('forward', type=str, \
    help='Forward primer sequence')
command.add_argument('probe', type=str, \
    help='Probe sequence')
command.add_argument('reverse', type=str, \
    help='Reverse primer sequence')
command.add_argument('database', type=argparse.FileType("r"), \
    help='Database to search on fasta')
...
class  GassstResult():
    ...
    def __repr__(self):
    	return self.match + " : " + str(self.start) + " in strand:" + str(self.strand) \
            + " in primer: " + self.primer +   " with error: " + str(self.error)

    def __eq__(self, other):
        return isinstance(other, GassstResult) and self.primer==other.primer \
            and self.match==other.match \
            and self.strand == other.strand and self.start==other.start
...
class  Amplicon():
    ...
    def __init__(self, forw, reve):
        if forw.strand == reve.strand:
            raise Exception("Amplicon could be in inverse strand for reverse/forward primer\n" + forw + reve )
        self.forw = forw
        self.prob = None
        self.reve = reve
    ...
    def contain_probe(self, probe):
        if min(probe.start,probe.stop) > self.get_start() \
    ...
    def get_tab_data(self):
        return (map(str, [self.get_start(), self.get_stop(), self.forw.error, self.prob.error, \
                          self.reve.error]))
    ...
    def __repr__(self):
        if self.prob is None:
            return str(self.get_start()) + " : " + str(self.get_stop()) + " in strand:" \
                + str(self.strand) + "; error F:" + str(self.forw.error) + ",R:"+ str(self.reve.error)
        return str(self.get_start()) + " : " + str(self.get_stop()) + " in strand:" \
            + str(self.strand) + "; error F:" + str(self.forw.error) + ",P:"+ str(self.prob.error) \
            + ",R:" + str(self.reve.error)
...
if __name__=='__main__':
    ...
    identity = int(100 - args.error / float(max(len(args.forward), len(args.reverse))) * 100)


    command = [gasst_exe, "-p", str(identity), "-g", "0", "-w", "6", '-h', '0', \
                   "-i", primer_fasta, "-d", args.database.name, "-o", output_result.name]
    ...
    sep = "\t"
    args.out.write("Id\tAmpli number\tDescription\tStart\tStop\tError forward\tError probe\tError reverse\n")
    for record in SeqIO.parse(args.database, 'fasta'):
        ...
        for i,amplicon in enumerate(amplicons.get(record.id)):
            ...
            args.out.write(record.id + sep + str(i+1) + sep + desc
                           + sep + sep.join(amplicon.get_tab_data()) + "\n")
    sys.stderr.write("OK\n")
    #delete tempfile
